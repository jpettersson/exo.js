(function() {
  var CSSTransitioner, Controller, DOMInflater, List, Model, Spine, _base, _base2, _base3, _base4,
  ...
  DOMInflater = {
    ...
    inflateFromDOM: function() {
      var attr, attributes, className, el, id, _i, _len, _ref;
      if (!this.modelClass) return;
      className = this.dashify(this.modelClass.className);
      id = this.el.attr("data-" + className + "-id");
      attributes = {
        id: id
      };
      _ref = this.modelClass.attributes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        attr = _ref[_i];
        if (el = this.el.find("[data-" + className + "-attribute='" + attr + "']")[0]) {
          attributes[attr] = el.innerText;
        }
      }
      return this.model = new this.modelClass(attributes);
    },
  ...
  if (typeof Exo !== "undefined" && Exo !== null) {
    (_base4 = Exo.Spine.Modules).DOMInflater || (_base4.DOMInflater = DOMInflater);
  }
