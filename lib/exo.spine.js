  ...
  DOMInflator = {
    ...
    */
    tagElements: function(collection) {
      ...
      for (_i = 0, _len = collection.length; _i < _len; _i++) {
        model = collection[_i];
        _results.push(this.findAndTagElement(model));
      }
      return _results;
    },
    /*
    ...
    */
    createControllers: function(collection) {
      ...
      for (_i = 0, _len = collection.length; _i < _len; _i++) {
        ...
        el = this.findAndTagElement(model);
    ...
    findAndTagElement: function(model) {
      var el;
      el = this.el.find("[data-" + (this.dashify(model.constructor.className)) + "-id=" + model.id + "]");
      $(el).data('item', model);
      return el;
    },
